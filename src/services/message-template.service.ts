...
class MessageTemplateService {
  ...
  async messageFindPlace(ctx: Context) {
    if (!('match' in ctx))
      return 'Error: messageFindPlace not found match in ctx';
...
${seatsMessage}
  ...
  generateDetailedTrainMessage(train: IRzdTrain) {
    ...
    const arrivalTime = new Date(train.LocalArrivalDateTime).toLocaleTimeString(
      'ru-RU',
      {
        hour: '2-digit',
        minute: '2-digit',
