...
const action = (bot: Telegraf) => {
    bot.on('message', async (ctx) => {
        ...
        if(!('text' in ctx.message)) {
            return ctx.reply('Сообщение пустое!');
        }

        axios.get<{data: ICity[]}>(`https://api.svrpk.ru/api/v1/suggest/stations?name=${ctx.message.text}`)
            .then(async ({data}) => {
                ...
                ctx.reply('Выберите город:', Markup.inlineKeyboard(cities, {columns: 1}));
            }).catch((error) => {
            console.log(error)
        });
    });
